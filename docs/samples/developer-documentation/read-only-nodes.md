...
This topic describes read-only nodes, [why they might be
required](#why-use-read-only-nodes), and [how to configure a node to be
read-only](#making-a-node-read-only).

A read-only node is one which, simply put, cannot be involved in any new
transactions. A read-only node has certain
[restrictions](#read-only-node-restrictions), and some functionality is
[disabled](#read-only-nodes-disabled-functionality).

## Why use read-only nodes?

Making a node read-only is a feature that is used for many reasons,
...
requirement to keep its data available for further use. Corda ledger
data is not easily interpreted outside of a Corda node because it is
stored as binary blobs in an AMQP encoding.

It is possible to interpret that data externally, but we currently
provide no such tooling. Keeping the node online in the network is not a
good option, as it continues to be an active participant in the network
and could participate in new transactions on the ledger.

It is possible to re-configure a node as read-only, with no ability to
mutate data or participate with peers but continue to perform vault
...
The purpose of this is so that database queries, which may take a long
time, do not block the database transactions traffic. This might happen
if only one node was used for transactions and for reporting. This
scenario keeps reporting workloads fully segregated from transactional
workloads and is an easy route to getting additional scalability.

## Read-only node restrictions

A read-only node has the following restrictions versus normal nodes:

- It is not visible on the peer-to-peer network.
...
- All its knowledge about the network and its participants comes from
  its configuration and database, as well as the network map cache.

- If its database is a replication of a live node, it can get fresh
  information about the network through the database.
...
- The node vault's produced states cache is disabled to prevent any
  stale data from being stored in memory.
...
restrict any unintentional mutating operations, it is recommended to
...
connection, but some database servers treat this only as a hint, and
